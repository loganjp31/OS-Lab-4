...
void memory_init(void) { /* Initialize your memory manager state. */
    ...
    freelist->start = 64; /* user memory starts at 64MB */
    freelist->size = 960; /* total user memory size */
    freelist->next = NULL;
}                       
...
int memory_allocate(process_t *p) {            /* Optional: allocate memory for a user process and set p->mem_start. */
    ...
    while (curr != NULL) {
        if (curr->size >= p->mem_req) {
            /* Found a block large enough to allocate */
            p->mem_start = curr->start; /* set process memory start */
            ...
            if (curr->size == 0) {
                ...
                if (prev == NULL) {
                    freelist = curr->next; /* removing head */
                } else {
                    prev->next = curr->next; /* bypass current block */
                }
                free(curr); /* free the block struct */
            }
...
int memory_free(process_t *p) {                /* Optional: free a user process memory block. */
    /* Create a new free block for the freed memory */
    ...
    if (prev == NULL) {
        freelist = new_block; /* Insert at head */
    } else {
        prev->next = new_block; /* Insert after prev */
    }

    /*Coalesce with next block if adjacent*/
    if (new_block->next != NULL && new_block->start + new_block->size == new_block->next->start) {
        new_block->size += new_block->next->size; /* merge sizes */
        free_block_t *temp = new_block->next; /* temp pointer to block being removed */
        new_block->next = new_block->next->next; /* bypass the next block */
        free(temp); /* free the merged block struct */
    }

    /*Coalesce with previous block if adjacent*/
    if (prev != NULL && prev->start + prev->size == new_block->start) {
        prev->size += new_block->size; /* merge sizes */
        free_block_t *temp = new_block; /* temp pointer to block being removed */
        prev->next = new_block->next; /* bypass the new block */
        free(temp); /* free the merged block struct */
    }
